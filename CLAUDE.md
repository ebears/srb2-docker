...
Docker Compose setup for running a Sonic Robo Blast 2 (SRB2) dedicated game server. The Dockerfile compiles SRB2 from source inside the container and the entrypoint script (`srb2.sh`) launches the dedicated server with optional mod support.
...
- `srb2.sh` — Container entrypoint that runs `lsdl2srb2 -dedicated` and optionally loads mods via `-file`
...
**Runtime flow:** `srb2.sh` is the entrypoint. It checks if `/mods` has files (using nullglob), filters to recognized types (`.wad`, `.pk3`, `.soc`, `.lua`, `.cfg`), then launches `lsdl2srb2 -dedicated -config adedserv.cfg -home /data`, appending `-file` with the mod paths when present. Extra arguments passed to the container (`docker run ... srb2-docker -maxplayers 16` or via Compose `command:`) are forwarded to `lsdl2srb2` before the `-file` mod args. The script also handles SIGTERM/SIGINT forwarding for graceful shutdown. On first run, the entrypoint copies the default `adedserv.cfg` to `/data/.srb2/`.

**Volumes:**
- `/mods` — Optional mods directory (`.wad`, `.pk3` files loaded automatically via `-file`)
...
The Dockerfile does not require game data files in the repository. The `gamedata` build stage automatically fetches the correct `Full.zip` from GitHub releases based on `SRB2_VERSION`, and extracts all `.dat` and `.pk3` files. The entrypoint validates the required files: `srb2.pk3`, `zones.pk3`, `characters.pk3`, `models.dat`, `music.pk3`.
